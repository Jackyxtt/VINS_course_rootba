...
namespace rootBA {
    ...
    size_t LandmarkBlock<Scalar>::ResizeStorage(void) {
        ...
        size_t cols = 6 + cameraNums * 6 + 3 + 1; //第一个6指的是T_bc
        size_t rows = cameraNums * 2 + 3;
        if (this->storage.rows() == rows) {//行数相等的话列也一定相等，不用重新指定矩阵的大小
...
//                 tempJp.template block<3, 3>(0, 3) = R_cb * R_bw * SkewSymmetricMatrix(p_w - t_wb);//右扰动
            ...
            ///------------- check jacobians -----------------
            {
                Vector3<Scalar> w(0.001, 0.001, 0.001);//四元数需要指定float类型才能用
                Eigen::Quaternionf delta_q; //Quaternionf:float Quaterniond:double
                delta_q.w() = 1;
                delta_q.vec() = 0.5 * w;
                delta_q.normalized();
                Matrix3<Scalar> R_bw_preturb = delta_q.toRotationMatrix().transpose() * R_bw;
                Vector3<Scalar> p_b_preturb = R_bw_preturb * (p_w - t_wb);
                Vector3<Scalar> p_c_preturb = R_cb * (p_b_preturb - t_bc);
                Vector2<Scalar> &origin_measure = observe.second->norm;
                Scalar invDep = 1.0 / p_c_preturb.z();
                Vector2<Scalar> norm_test = p_c_preturb.template head<2>() / p_c_preturb.z();
                Vector2<Scalar> residual_preturb;
                residual_preturb = norm_test - origin_measure;
                residual_preturb = sqrtInfo * residual_preturb;
                if (print_once){
//                    std::cout << "-------------------checkJacobian---------------------" << std::endl;
//                    std::cout << "check Jp" << std::endl;
//                    std::cout << "residual diff = \n" << residual_preturb << std::endl;
...
//                    std::cout << "-------------------end checkJacobian---------------------" << std::endl;
                    ...
                    ofs << residual_preturb << std::endl;

                    Vector2<Scalar> residual_before_preturb = p_c.template head<2>() / p_c.z() - origin_measure;
                    ...
                    ofs << "residual diff = \n" <<
                    residual_preturb - residual_before_preturb << std::endl;
                    ofs << "Jacobian diff = \n" << reduce * tempJp.template block<3, 3>(0, 3) * w << std::endl;
                    ofs.close();
                    print_once = false;
                }
...
//        std::cout << "Jl after givens" << this->storage.block(0, cols - 4, rows, 3) << std::endl;

//        查看givens变化是否正确
//        std::ofstream outfile;
//        outfile.open("./src/debug/Jl.txt", std::ios::app|std::ios::out);
//        outfile << this->storage.block(0, cols - 4, rows, 3);
//        outfile.close();
